...
public class ChocoAutomatonBuilder {
    ...
    public static MealyAutomaton build(ScenarioTree tree, int size, boolean isComplete) {
        return build(tree, size, isComplete, false, null);
    }

    public static MealyAutomaton build(ScenarioTree tree, int size, boolean isComplete, boolean isWeakComplete) {
    ...
    public static List<MealyAutomaton> buildAll(ScenarioTree tree, int size, boolean isComplete, boolean isWeakComplete,
                                                PrintWriter modelPrintWriter) {
        IntegerVariable[] nodesColorsVars = Choco.makeIntVarArray("Color", tree.nodeCount(), 0, size - 1);
        CPModel model = buildModel(tree, size, isComplete, isWeakComplete, nodesColorsVars, modelPrintWriter);
    ...
    private static Constraint[] getCompleteConstraints(CPModel model, int size, ScenarioTree tree,
        ...
        Map<String, Map<String, List<MealyNode>>> eventExprToNodes = new TreeMap<>();

        Map<String, List<MyBooleanExpression>> pairs = tree.pairsEventExpression();
        for (String event : pairs.keySet()) {
            Map<String, List<MealyNode>> exprMap = new TreeMap<>();
        ...
        for (String event : pairs.keySet()) {
            for (MyBooleanExpression expr : pairs.get(event)) {
                List<MealyNode> nodes = eventExprToNodes.get(event).get(expr.toString());
...
/*
    ...
    private static List<MealyAutomaton> buildAllAutomatonsFromModel(int size, ScenarioTree tree,
        ...
        List<MealyAutomaton> ans = new ArrayList<>();
        if (!solver.existsSolution()) {
            return ans;
        }

        do {
            MealyAutomaton automaton = new MealyAutomaton(size);
            for (int i = 0; i < tree.nodeCount(); i++) {
                int color = solver.getVar(nodesColorsVars[i]).getVal();
                MealyNode state = automaton.state(color);
    ...
    private static MealyAutomaton buildAutomatonFromModel(int size, ScenarioTree tree, IntegerVariable[] nodesColorsVars,
                                                          CPModel model) {
        CPSolver solver = new CPSolver();
        solver.read(model);
        solver.setTimeLimit(300000);
...
//                  new IncreasingDomain()));
        ...
        MealyAutomaton ans = null;
        if (solver.existsSolution()) {
            ans = new MealyAutomaton(size);
            for (int i = 0; i < tree.nodeCount(); i++) {
                int color = solver.getVar(nodesColorsVars[i]).getVal();
