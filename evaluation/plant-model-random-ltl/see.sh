...
for size in 10 15 20 25 30 35; do
    finished=$(grep -nr " FOUND" $dir/$size-*/*.full.log | wc -l)
    ...
    if (( finished == 0 )); then
        q2=0
    elif (( finished % 2 == 0 )); then
        left=${arr[$(($finished / 2 - 1))]}
        right=${arr[$(($finished / 2))]}
        q2=$(python -c "print($left / 2 + $right / 2)")
    else
        q2=${arr[$(($finished / 2))]}
    fi
    echo "Median iterations: $q2; Q3: ${arr[$(($finished * 3 / 4))]}"
done
