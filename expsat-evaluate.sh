        ...
        for ((events = 2; events <= 5; events++)); do
            for ((actions = 2; actions <= 5; actions++)); do
                ...
                for i in 20 40 80 160; do
                    ...
                    echo ">>> $fullname"
                    rm -f "$fsm"
                    java -Xms2G -ea -jar jars/qbf-automaton-generator.jar "$fullname" --ltl "$name-$suffix.ltl" --size "$size" --timeout "$timeout" --depth "$depth"  --solverParams "$solver_params" --complete --result "$fsm" --strategy SAT 2>&1 | grep "\\(INFO\\|WARNING\\|SEVERE\\|Exception\\)"
                    if [ -f "$fsm" ]; then
