        ...
        for ((events = 2; events <= 5; events++)); do
            for ((actions = 2; actions <= 5; actions++)); do
                ...
                for i in 20 40 80 160; do
                    ...
                    echo ">>> $fullname"
                    rm -f "$fsm"
                    java -ea -jar jars/qbf-automaton-generator.jar "$fullname" --ltl "$name-$suffix.ltl" --size "$size" --timeout "$timeout" --depth "$size" -qs SKIZZO  --solverParams "$solver_params" --complete --bfsConstraints --result "$fsm" 2>&1 | grep "\\(INFO\\|WARNING\\|SEVERE\\|Exception\\)"
                    if [ -f "$fsm" ]; then
